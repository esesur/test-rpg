...
public class TablePrinter {
    ...
    public void print(List<String> headers, List<List<String>> rows) {
        if (headers.size() != rows.get(0).size()) throw new IllegalArgumentException("Number of columns is not equal.");
        ...
        for (int i = 0; i < headers.size(); ++i) {
            System.out.printf(verticalDelimiter + indent + "%-" + maxElementLengths[i] + "s" + indent, headers.get(i));
        }
        ...
        for (int i = 0; i < rows.size(); ++i) {
            ...
            for (int j = 0; j < columns.size(); ++j) {
                System.out.printf(verticalDelimiter + indent + "%-" + maxElementLengths[j] + "s" + indent, columns.get(j));
            }
            System.out.println(verticalDelimiter);
        }
        printLine();
    ...
    private void updateMaxLengthsFromList(List<String> elements) {
        ...
        for (int i = 0; i < elements.size(); ++i) {
            s = elements.get(i);
            if (s.length() > maxElementLengths[i])
                maxElementLengths[i] = s.length();
        }
    }
